...
suite "CUDA: Shapeshifting - broadcasting and non linear algebra elementwise operations":
  ...
  test "Implicit tensor-tensor broadcasting - basic operations +., -., *., ./, .^":
    ...
    block: # Float division
      ...
      # fp32 division makes exact `==` fail (no true FP64). The float32 add/sub/mul
      # blocks above still run. See corex_port/blockers.json (terminal FP64).
  ...
  test "Implicit tensor-tensor broadcasting - basic in-place operations +.=, -.=, *.=, /.=":
    ...
    block: # Float division
      # Note: We can't broadcast the lhs with in-place operations
      # CoreX ivcore11 port: this block is float64 (no asType(float32)); device
      # fp32 division makes exact `==` fail (no true FP64). The float32 blocks above
      # (incl. integer division cast to float32) still run. See corex_port/blockers.json.
      when false: # FP64 not accurate on ivcore11
